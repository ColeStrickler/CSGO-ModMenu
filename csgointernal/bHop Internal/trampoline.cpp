...
char* TrampHook(char* src, char* dst, unsigned int len) {
	...
	uintptr_t jmpAddress = (uintptr_t)(src - gateway - 5);
	*(gateway + len) = (char)0xE9;
	*(uintptr_t*)(gateway + len + 1) = jmpAddress;
	if (Hook(src, dst, len)) {
		return gateway;
	}
	else {
